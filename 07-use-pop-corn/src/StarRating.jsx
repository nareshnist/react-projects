...
export default function StarRating({ maxRating = 5, color = '#fcc419', size = 48 ,className,messages =[] , defaultRating = 0 ,onSetRating}) {
    ...
    const handleOnMoveEnter = (rating) => {
        setTempRating(rating)
        onSetRating(rating)
    }

    const handleOnMoveLeave = () => {
        setTempRating(0)
    }
    ...
    return (
        <div style={containerStyle}>
            <div style={starContainerStyle} className={className}>
                {
                    Array.from({ length: maxRating }).map((_, i) => <Star key={i} full={tempRating ? tempRating >= i + 1 : rating >= i + 1} rating={rating} onRate={() => {
                        setRating(i + 1);
                        console.log("clicked");

                    }}
                        onHoverIn={() => handleOnMoveEnter(i + 1)}
                        onHoverOut={() => handleOnMoveLeave(0)}
                        color={color}
                        size={size}
                    />)
                }
